...
- **Background/Tasks/MessageTaskResult**: kind-aware background-work tracking (subagents, backgrounded shell commands), replacing v0.8.0's `Message.Subagents`/`SubagentStats`/`MessageSubagentResult` outright (greenfield break, no shim — full ADR set in `docs/designs/2026-07-13-issue-61-background-shell-tasks.md`). `MessageTaskResult` is a background task's terminal result nested inside the parent turn; it never ends the turn (only the parent's own `MessageResult` does).
  ...
  - **Tracking gates** (`engine/cli`): `cli.WithSubagentTools(names...)` → `BackgroundSubagent` (+ name-based fallback pre-snapshot); `cli.WithShellTracking()` → `BackgroundShell` and any other native-feed kind. Independent switches; parsing `Message.Tasks` is unconditional regardless of either.
...
- **InitMeta**: Init-time metadata (`Model`, `AgentName`, `AgentVersion`) set exclusively on `MessageInit`. Nil on all other message types. Nil-guard: only non-nil when at least one field contains meaningful data (matching `*Usage` discipline). All fields use `omitempty` strings. Claude CLI populates `Model` and `AgentVersion` (from `claude_code_version`); `AgentName` stays empty. ACP populates all 3 fields (from `initializeResult.AgentInfo` and `sessionModelState.CurrentModelID`). OpenCode/Codex: `Init` stays nil (no init metadata in wire format). All 3 fields sanitized via `errfmt.SanitizeCode` at parse time (control-char rejection + 128-byte cap).
