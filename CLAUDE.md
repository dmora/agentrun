...
- **ContextUsedTokens on CLI MessageResult**: The CLI engine derives `ContextUsedTokens` from the peak per-call input-side fill (`max(InputTokens + CacheReadTokens + CacheWriteTokens)`) across non-result messages in the turn. Only set when the backend emits per-call usage (e.g., Claude CLI assistant events). Backends that report usage only on the result event (Codex, OpenCode) leave this field at 0 — the engine intentionally does not fabricate a value from result-level totals. ACP's `usage_update` provides authoritative context fill and is unaffected. Note: `ContextUsedTokens` uses `omitempty`, so 0 is always omitted in JSON. On ACP `MessageContextWindow` where 0 means "empty context window" (fresh session), consumers distinguish "reported but empty" from "not reported" by checking `ContextSizeTokens > 0` on the same message.
