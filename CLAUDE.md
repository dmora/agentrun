...
- **MessageContextWindow**: Mid-turn context window fill state. Only `ContextSizeTokens` and `ContextUsedTokens` are meaningful on this message type; other Usage fields are zero and should be ignored. Cost data intentionally excluded (CostUSD is authoritative only on MessageResult to avoid double-counting).
  ...
  - **CLI**: synthesized by the engine after mid-turn content messages (e.g., `MessageText`, `MessageThinking`) with `ContextUsedTokens` or `ContextSizeTokens` populated. Not synthesized for `MessageResult`, `MessageInit`, or `MessageError` (turn-boundary types). The guard condition delegates to `ContextFill()` to avoid dual ownership of the "has fill data" predicate. The content message retains its original Usage for backward compatibility; the synthesized `MessageContextWindow` carries only context fill fields. Consumers should not double-count — prefer `MessageContextWindow` for context fill monitoring. Use `ContextFill(msg)` helper for extraction.
- **ContextUsedTokens on CLI messages**: The CLI engine populates `ContextUsedTokens` on two message scopes:
  ...
  - After enrichment, the CLI engine emits a synthesized `MessageContextWindow` following each mid-turn message that has context fill data. This unifies the context fill signal across CLI and ACP backends.
  - Note: `ContextUsedTokens` uses `omitempty`, so 0 is always omitted in JSON. On ACP `MessageContextWindow` where 0 means "empty context window" (fresh session), consumers distinguish "reported but empty" from "not reported" by checking `ContextSizeTokens > 0` on the same message.
...
- **Sanitization convention**:
  - Structured metadata/identifier fields on Message (`ErrorCode`, `StopReason`, `InitMeta.*`, `PermissionDenial.Tool`) are sanitized at parse time via `errfmt.SanitizeCode` (reject-on-control, 128-byte cap).
  - `PermissionDenial.Reason` is sanitized via `errfmt.Truncate` (4096-byte cap).
