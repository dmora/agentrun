...
- **Context-fill reporting on CLI backends**: The CLI engine enriches messages with `ContextUsedTokens` and synthesizes `MessageContextWindow` messages to provide a unified context-fill signal.
  - **`ContextUsedTokens` enrichment**: The engine populates `ContextUsedTokens` on two message scopes:
    ...
    - **Subagent event filtering**: Claude CLI's JSONL stream includes events from subagents (Agent tool) alongside parent events. Subagent events are identified by `parent_tool_use_id != null` in the raw JSON. The Claude parser nil's out `Usage` on subagent events to prevent their context fill (which reflects the subagent's smaller/different context window) from inflating the parent's `maxCallFill` tracker. Raw JSON is preserved in `msg.Raw` for consumers who need per-subagent usage.
